% this is the master file that joins all steps to analyze citometry data to
...
%         Media(m).robfit(:,w) = robfit;
        ...
        ylim( [-1.3 2.8] )
        xlim( [.5 20.5] )
        title( strcat(cepas(w),'-Well:',num2str(w)) )
        set( gca,'xticklabel',[] )
